...
from ..storage.models import User, create_tables, set_data_path
...
def _add_repository(name, url, path):
    origin = git.Repo(url)
    ...
    if path.exists() and path.is_dir():
        ...
        remote.fetch()
    __repositories[name] = origin, repo, str(url)


def _get_heads_of_repositories():
    ret = {}
    for name, (origin, repo, url) in __repositories.items():
...
def get_system_info():
    ...
    info['repositories'] = _get_heads_of_repositories()
    return info
...
def clean_side_effects(task: Task,
    ...
    return executor.feed(task.id, -2, cmds, next_feed_time=0)
    ...
    try:
        while True:
            ...
            if (feed_finished and
                ...
                executor.save(task.id,
                              f'{task.data_path}/{task.record.id}',
                              task.result()['index'])
...
def bootstrap(executor: Executor,
    ...
    if repositories is not None:
        for name, url in repositories.items():
            _add_repository(name, url, __data_path / 'repositories')
